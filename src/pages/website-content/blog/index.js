...
import TableBlog from 'src/views/tables/TableBlog'
import Dialogblog from 'src/views/dialog/Dialogblog'
...
const BlogPage = () => {
    ...
    const [blogFormDialogOpen, setBlogFormDialogOpen] = useState(false)
    const [blogFormDialogMode, setBlogFormDialogMode] = useState('add')
    const [blogToEdit, setBlogToEdit] = useState(null)

    const toggleblogFormDialog = (e, mode = 'add', blogToEdit = null) => {
        setBlogFormDialogOpen(prev => !prev)
        setBlogFormDialogMode(mode)
    ...
    const fetchData = ({ currentPage, pageSize = DefaultPaginationSettings.ROWS_PER_PAGE, search }) => {
        ...
        axiosInstance
            .get(ApiEndPoints.BLOG.list, { params })
            .then(response => {
                setcareerData(response.data.data.blog)
    ...
    const onConfirmDelete = useCallback(
        e => {
            ...
            axiosInstance
                .delete(ApiEndPoints.BLOG.delete(careerToDelete._id))
        ...
        <>
            <Grid container spacing={4} className='match-height'>
                <PageHeader
                    ...
                    action={
                        <Button variant='contained' onClick={toggleblogFormDialog}>
                            Add Blog
                        </Button>
                    }
                />
                    ...
                    <Card>
                        ...
                        <Box sx={{ p: 5 }}>
                            <TableBlog
                                ...
                                pageSize={pageSize}
                                toggleEdit={toggleblogFormDialog}
            ...
            <Dialogblog
                mode={blogFormDialogMode}
                open={blogFormDialogOpen}
                toggle={toggleblogFormDialog}
                dataToEdit={blogToEdit}
                onSuccess={() => {
                    fetchData({
                        currentPage: currentPage,
                        pageSize: pageSize
