...
const validationSchema = yup.object().shape({
    ...
    //         })
    //     )
    //     .min(1, 'At least one Program Name and Code is required'),
    // programs: yup.array()
    //     .of(
    //         yup.object({
    //             program: yup.string().required('Program is required'),
    //             numberOfProgram: yup.string().required('Number of Program is required'),
    //         })
    //     )
    //     .min(1, 'At least one Program and Number Of Program is required'),

    // yearEstablished: yup.string().trim().required('Year is required').nullable().notRequired(),
    // totalAlumniCount: yup.string().trim().required('Total Count is required'),
    // currentEnrollment: yup.string().trim().required('Current Enrollment is required'),
...
const CollegeForm = () => {
        ...
        axiosInstance
            ...
            .then((response) => {
                const states = response.data.listData.map((item, index) => ({
                    id: index + 1, // Add unique ID for Autocomplete
                    name: item.state, // Update this if state field is different
                }));
                setStateList(states);
                setCityList([]);
            })
    ...
    const fetchEditData = async () => {
        ...
        reset({
            ...
            totalAlumniCount: data?.totalAlumniCount || null,
            programCount: data?.programCount || null,
            isActiveAlumniAssociations: data?.isActiveAlumniAssociations || false,
            preferredFeatures: data?.preferredFeatures
                ? data.preferredFeatures.split(',') // Split the string into an array
                : [],
            integrationNeeds: data?.integrationNeeds
                ? data.integrationNeeds.split(',') // Split the string into an array
                : [],
            isDataPrivacyAgreement: data?.isDataPrivacyAgreement || false,
            isTermsAndConditions: data?.isTermsAndConditions || false,
            social_media_links: {
            ...
            // collegePrograms: data?.collegePrograms?.length
            //     ? data.collegePrograms.map((item) => ({
            ...
            isTermsAndConditions: data?.isTermsAndConditions || false,
            logo: data?.logo || null,
            brandingFile: data?.brandingFile || null,
            isExternalDbExist: data?.isExternalDbExist || false,
            dbHostName: data?.isExternalDbExist ? data.dbHostName || null : null,
            dbUserName: data?.isExternalDbExist ? data.dbUserName || null : null,
            dbPassword: data?.isExternalDbExist ? data.dbPassword || null : null,
            databaseName: data?.isExternalDbExist ? data.databaseName || null : null,
        });

        if (data?.country) {
            await fetchStateData(data.country); // Fetch states for the selected country
        }
        if (data?.state) {
            await fetchCityData(data.state, data.country); // Fetch cities for the selected state
        }
    };
        ...
        if (data) {
            setValue(
                ...
                data?.preferredFeatures
                    ? data.preferredFeatures.split(',')
                    : []
            );
            setValue(
                'integrationNeeds',
                data?.integrationNeeds
                    ? data.integrationNeeds.split(',')
                    : []
            );
        }
    }, [data, setValue]);
    ...
    useEffect(() => {
        if (data) {
            setPrograms(data?.collegePrograms || []);
            setValue('collegePrograms', data?.collegePrograms || []);
        }
    }, [data, setValue]);

    useEffect(() => {
        setLoading(false);
        setTitle(mode === "add" ? "Add College" : "Edit College");
        fetchEditData()
    ...
    const onSubmit = async (data) => {
        ...
        // if (data.totalAlumniCount) payload.append('totalAlumniCount', data.totalAlumniCount);

        // Add combined collegePrograms to payload
        // if (programs?.length) {
        //     programs.forEach((program, index) => {
        //         payload.append(`collegePrograms[${index}][programName]`, program.programName);
        //         payload.append(`collegePrograms[${index}][programCount]`, program.programCount);
        //     });
        // }
        ...
        if (data.submittedBy) payload.append('submittedBy', data.submittedBy && data.submittedBy.trim() !== "" ? data.submittedBy : null);
        ...
        } else if (mode === "edit" && typeof data.logo === "string") {
        ...
        } else if (mode === "edit" && typeof data.brandingFile === "string") {
        ...
        if (mode === "edit") {
            apiInstance = axiosInstance
                .post(ApiEndPoints.COLLEGE.edit, payload, {
                    headers: {
            ...
            apiInstance = axiosInstance
                .post(ApiEndPoints.COLLEGE.create, payload, {
                    headers: {
                        'Content-Type': 'multipart/form-data'
                    }
                })
    ...
    const handleAddProgram = () => {
        if (programInput.trim() && programInput.trim()) {
            setValue('programs', [
                ...programs,
                {
                    programName: programInput.trim(),
                    programCount: programNumberInput?.trim(),
                }
            ])
            ...
            setPrograms(prevProgram => [
                ...
                {
                    programName: programInput.trim(),
                    programCount: programNumberInput?.trim() || null
                },
            ]);
    ...
    const handleDeleteProgram = (programToDelete) => {
        setPrograms((prevProgram) => {
            const updatedPrograms = prevProgram.filter((_, index) => index !== programToDelete);
                    ...
                    <Grid container spacing={4}>
                        ...
                        <Grid item xs={4}>
                            <FormControl fullWidth>
                                <FormLabel required htmlFor='orgName'>College Name</FormLabel>
                                <Controller
                                    name="orgName"
                                    control={control}
                        ...
                        <Grid item xs={12} pb={3}>
                            <FormControl fullWidth>
                                <FormLabel
                                    htmlFor="programs"
                                >
                                    Number of Programs Offered (Eg. Undergraduate, Postgraduate, etc.)
                                </FormLabel>
                                <Controller
                                    name="programs"
                                    control={control}
                                            ...
                                            <Grid container spacing={2}>
                                                ...
                                                <Grid item xs={11} md={5.5}>
                                                    <TextField
                                                        ...
                                                        variant="outlined"
                                                        onChange={(e) => setProgramNumberInput(e.target.value)}
                                                        id="programCount"
                                                ...
                                                <Grid item xs={1} md={1}>
                                                    <InputAdornment position="end">
                                                        <Button disableRipple onClick={handleAddProgram} sx={{ mt: 10 }} variant="contained" fullWidth>
                                                            Add
                                                        </Button>
                                                    </InputAdornment>
                                                </Grid>
                                        ...
                                        >
                                            <Box sx={{ display: "flex", gap: 2 }}>
                                                ...
                                                <Box > <Typography sx={{ fontSize: "14px" }}>
                                                    <span style={{ fontWeight: 'bold' }}>Program Offered</span> : {desc.programName}
                                                </Typography>
                                                    <Typography sx={{ fontSize: "14px" }}>
                                                        <span style={{ fontWeight: 'bold' }}>Number Of Programs</span> : {desc.programCount}
                                                    </Typography></Box>

                                            </Box>

                                            <IconButton onClick={() => handleDeleteProgram(index)}>
                                                <CloseIcon sx={{ fontSize: "16px", color: "#602de1c2" }} />
                                            </IconButton>
                                        </Box>
                                    ))}
                                </Box>
                        ...
                        {/* <Grid item xs={6}>
                            <FormControl fullWidth>
                                <FormLabel required htmlFor='currentEnrollment'>Current enrolled Students</FormLabel>
                                <Controller
                                    ...
                                    render={({ field: { value, onChange } }) =>
                                        <TextField
                                            type="text"
                                            placeholder='Enter Current enrolled Students'
                                            onChange={onChange}
                                            id='currentEnrollment'
                                            value={value}
                                            error={Boolean(errors.currentEnrollment)}
                                        />
                                    }
                                />
                        ...
                        <Grid item xs={6}>
                            <FormControl fullWidth>
                                <FormLabel required htmlFor='isActiveAlumniAssociations'>Active Alumni Associations</FormLabel>
                                <Controller
                                    name="isActiveAlumniAssociations"
                                    control={control}
                                    defaultValue={data?.isActiveAlumniAssociations || false}
                                    render={({ field: { value, onChange } }) =>
                                        <Select
                        ...
                        {mode === 'edit' ?
                            <>
                                        ...
                                        <Controller
                                            ...
                                            render={({ field: { value, onChange } }) =>
                                                <TextField
                                                    ...
                                                    defaultValue={0}
                                                    inputMode="numeric"
                                                    placeholder='Enter Total Alumni Count'
                                                    onChange={onChange}
                                                    id='totalAlumniCount'
                                                    value={value}
                                                    disabled
                                ...
                                <Grid item xs={12} sm={6}>
                                    <FormControl fullWidth>
                                        <FormLabel htmlFor='programCount'>Program Count</FormLabel>
                                        <Controller
                                            name="programCount"
                                            control={control}
                                            rules={{ pattern: /^[0-9]+$/ }}
                                            render={({ field: { value, onChange } }) =>
                                                <TextField
                                                    ...
                                                    defaultValue={0}
                                                    inputMode="numeric"
                                                    placeholder='Enter Program Count'
                                                    onChange={onChange}
                                                    id='programCount'
                                                    value={value}
                                                    disabled
                        ...
                        <Grid item xs={12}>
                            <FormControl fullWidth>
                                ...
                                <Controller
                                    ...
                                    defaultValue={
                                        data?.preferredFeatures
                                            ? data.preferredFeatures.split(',').map((item) => item.trim().toLowerCase().replace(/ /g, '_'))
                                            : []
                                    }
                        ...
                        <Grid item xs={12}>
                            <FormControl fullWidth>
                                ...
                                <Controller
                                    ...
                                    defaultValue={
                                        data?.integrationNeeds
                                            ? data.integrationNeeds.split(',').map((item) => item.trim().toLowerCase().replace(/ /g, '_'))
                                            : []
                                    }
                                    rules={{ required: 'At least one feature must be selected.' }}
                                    render={({ field: { value = [], onChange } }) => (
                        ...
                        <Grid item xs={12}>
                            <FormControl component="fieldset" fullWidth>
                                <Controller
                                    ...
                                    control={control}
                                    defaultValue={data?.isExternalDbExist}
                                    render={({ field: { value, onChange } }) => (
                                        <RadioGroup
                                            row
                                            value={value}
                        ...
                        {/* <Grid item xs={4}>
                            <FormControl fullWidth>
                                <FormLabel required htmlFor='confirmationEmail'>Email</FormLabel>
                                <Controller
                                    ...
                                    render={({ field: { value, onChange } }) =>
                                        <TextField
                                            ...
                                            value={value}
                                            error={Boolean(errors.confirmationEmail)}
                                        />
                                    }
                                />
                        ...
                        <Grid item xs={4}>
                            <FormControl fullWidth>
                                <FormLabel required htmlFor='confirmationPhone'>Phone Number</FormLabel>
                                <Controller
                                    ...
                                    render={({ field: { value, onChange } }) =>
                                        <TextField
                                            ...
                                            inputMode="numeric"
                                            placeholder='Enter Phone Number'
                                            error={Boolean(errors.confirmationPhone)}
                                            options={{
                                                phone: true,
                                ...
                                />
                                {errors.confirmationPhone && (
                                    <FormHelperText sx={{ color: 'error.main' }}>
                                        {errors.confirmationPhone.message}
                                    </FormHelperText>
                                )}
                            </FormControl>
                        </Grid> */}
