...
const DialogCountry = (props) => {
    ...
    useEffect(() => {
        if (open) {
            setLoading(false);
            setDialogTitle(mode === "add" ? "Addd Country" : "Edit Country");
            reset({
    ...
    const onSubmit = (data) => {
        ...
        let payload = {
            name: data.name,
            ...(mode === "edit" && { countryRecordId: dataToEdit.id })
        }
        setLoading(true);
        let apiInstance = null;
        if (mode === "edit") {
            apiInstance = axiosInstance
                .post(ApiEndPoints.COUNTRY.edit, payload, {
                })
        } else {
            apiInstance = axiosInstance
                .post(ApiEndPoints.COUNTRY.create, payload, {
                })
        }
        apiInstance
