...
const DialogCompany = (props) => {
    ...
    useEffect(() => {
        if (open) {
            setLoading(false);
            setDialogTitle(mode === "add" ? "Add Company" : "Edit Company");
            reset({
    ...
    const onSubmit = (data) => {
        ...
        let payload = {
            name: data.name,
            ...(mode === "edit" && { companyId: dataToEdit.id })
        }
        setLoading(true);
        let apiInstance = null;
        if (mode === "edit") {
            apiInstance = axiosInstance
                .post(ApiEndPoints.COMPANY.edit, payload, {
                })
        } else {
            apiInstance = axiosInstance
                .post(ApiEndPoints.COMPANY.create, payload, {
                })
        }
        apiInstance
