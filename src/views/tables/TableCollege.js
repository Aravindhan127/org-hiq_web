...
function TableCollege({
    ...
    const hasEditPermission =
        rolePremission?.permissions?.some(item => item.permissionName === 'college.edit') || isMasterAdmin === true;

    const hasActiveDeactivePermission =
        rolePremission?.permissions?.some(item => item.permissionName === 'college.activedeactive') || isMasterAdmin === true;
    ...
    return <CustomDataGrid
        ...
        columns={[
                ...
                : []),
            {
                ...
                renderCell: ({ row }) =>
                    <IconButton size="small" color="primary" variant="outlined" onClick={() => navigate(`/college/${row?.orgId}`)} >
                        <RemoveRedEyeIcon />
                    </IconButton>
            },
        ]}
